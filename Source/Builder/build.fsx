...
let ProjectName = "Coinbase.Commerce";
let GitHubUrl = "https://github.com/bchavez/Coinbase.Commerce"

let Folders = Setup.Folders(workingDir)
let Files = Setup.Files(Folders)
let Projects = Setup.Projects(ProjectName, Folders)

let CoinbaseProject = NugetProject("Coinbase.Commerce", "Coinbase Commerce API for .NET", Folders)
...
//                        "SignAssembly", BuildContext.IsTaggedBuild.ToString()
    ...
    CopyDir (CoinbaseProject.OutputDirectory @@ tag) CoinbaseProject.MsBuildFolderRelease allFiles
